    ...
    <div className="flex flex-col min-h-screen">
      ...
      <TabsContent value="reports" className="mt-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
          {reports.length > 0 ? (
            reports.map((report) => (
              <div key={report._id} className="border border-border/50 rounded-xl m-3 overflow-hidden bg-accent/10 hover:bg-accent/20 transition-colors">
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <UserAvatar user={report.reportedBy} size="xs" />
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-foreground truncate">@{report.reportedBy?.username}</p>
                  ...
                  {report.postId && (
                    <>
                      <div className="bg-background/40 p-3 rounded-lg border border-border/30 mb-2">
                        <p className="text-sm line-clamp-3 whitespace-pre-wrap">{report.postId.content}</p>
                      </div>
                      <div className="flex items-center gap-2 mb-3">
                        <UserAvatar user={report.postId.author} size="xs" />
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-foreground truncate">{report.postId.author?.name}</p>
                          <p className="text-[10px] text-muted-foreground">@{report.postId.author?.username}</p>
                        </div>
                      </div>
                    </>
                  )}
                ...
                <div className="flex gap-2 px-4 pb-4">
                  <Button
                    ...
                    variant="outline"
                    className="flex-1 text-[10px] font-bold h-9 bg-background border-border/50 hover:bg-green-500/10 hover:text-green-400 hover:border-green-500/30 uppercase tracking-tight"
                    onClick={() => report.postId && handleContentAction(report.postId._id, 'delete')}
                  >
