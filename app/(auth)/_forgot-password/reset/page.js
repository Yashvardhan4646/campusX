...
import { Eye, EyeOff, Loader2, Check, X, ShieldCheck, Lock } from 'lucide-react';
...
export default function ResetPasswordPage() {
  ...
  const getStrengthLevel = () => {
    ...
    if (score === 0) return { label: 'Empty', color: 'bg-muted', text: 'text-muted-foreground' };
    if (score === 1) return { label: 'Weak', color: 'bg-red-500', text: 'text-red-500' };
    if (score === 2) return { label: 'Medium', color: 'bg-yellow-500', text: 'text-yellow-500' };
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-background w-full p-4">
        <Card className="w-full max-w-md border-green-500/20 bg-green-500/5 backdrop-blur-md text-center p-8">
          ...
          <div className="bg-background/50 rounded-lg py-3 px-4 inline-block text-xs font-medium text-muted-foreground">
            Redirecting to login in <span className="text-foreground font-bold">{countdown}s</span>...
          </div>
        </Card>
      </div>
    );
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-background w-full p-4">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground mt-2 text-sm">Secure your account</p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="relative">
                  ...
                  <Input 
                    ...
                    className={`pl-10 h-11 bg-background/50 transition-all focus:ring-4 focus:ring-primary/10 ${
                      passwords.confirmPassword && passwords.newPassword !== passwords.confirmPassword ? 'border-red-500/50' : ''
                    }`}
                    placeholder="••••••••"
                    required 
                  />
              ...
              <div className="space-y-2 pt-2">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Strength</span>
                  <span className={`text-[10px] font-bold uppercase ${strLevel.text}`}>{strLevel.label}</span>
                </div>
                <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden flex gap-1">
                  <div className={`h-full flex-1 transition-all duration-500 ${strength.length || strength.uppercase || strength.number ? strLevel.color : 'bg-muted'}`} />
                  <div className={`h-full flex-1 transition-all duration-500 ${ (strength.length && strength.uppercase) || (strength.length && strength.number) || (strength.uppercase && strength.number) ? strLevel.color : 'bg-muted'}`} />
                  <div className={`h-full flex-1 transition-all duration-500 ${strength.length && strength.uppercase && strength.number ? strLevel.color : 'bg-muted'}`} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 p-3 rounded-xl bg-accent/20 border border-border/50">
                <Requirement met={strength.length} label="Min 8 characters" />
                <Requirement met={strength.uppercase} label="At least 1 uppercase" />
                <Requirement met={strength.number} label="At least 1 number" />
              </div>
              
              <Button type="submit" className="w-full rounded-full h-11 text-sm font-bold shadow-lg shadow-primary/10 transition-all hover:scale-[1.02] active:scale-[0.98]" disabled={loading}>
...
function Requirement({ met, label }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-xs ${met ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>{label}</span>
    </div>
  );
}
