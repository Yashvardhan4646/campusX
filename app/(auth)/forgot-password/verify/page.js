...
export default function VerifyOtpPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(`/forgot-password/reset?token=${data.resetToken}&email=${encodeURIComponent(email)}`);
    } catch (err) {
      toast.error(err.message);
    } finally {
      setLoading(false);
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-background w-full p-4">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground mt-2 text-sm">Security Verification</p>
        </div>
        ...
        <Card className="border-border/50 shadow-2xl shadow-primary/5 bg-card/50 backdrop-blur-sm">
          <CardHeader className="pb-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              We&apos;ve sent a 6-digit code to <span className="text-foreground font-semibold">{maskEmail(email)}</span>.
            </p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="text-center">
                {timer > 0 ? (
                  <p className="text-xs text-muted-foreground">
                    Resend code in <span className="text-foreground font-mono font-bold">{timer}s</span>
                  </p>
                ) : (
                  <button
                    type="button"
              ...
              <Button type="submit" className="w-full rounded-full h-11 text-sm font-bold shadow-lg shadow-primary/10" disabled={loading}>
